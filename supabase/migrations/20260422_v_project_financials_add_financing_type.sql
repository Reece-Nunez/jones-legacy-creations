...
COMMENT ON VIEW v_project_financials IS
  'Source of truth for per-project totals. Accrued interest is time-dependent so stays in app code (lib/finance/project-financials.ts). financing_type drives whether origination_fee/interest are revenue (seller_financed) or cost (external_loan) — reports should branch on it.';
