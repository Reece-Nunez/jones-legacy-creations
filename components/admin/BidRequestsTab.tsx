...
export function BidRequestsTab({
  ...
  async function sendBlast(e: React.FormEvent) {
    ...
    const recipients = contractors
      ...
      .map((c) => ({ contractor_id: c.id, name: c.name, email: c.email, phone: c.phone }));
  ...
  async function act(bid: BidRequest, action: "accept" | "reject" | "complete" | "void") {
    setBusy(true);
    try {
      ...
      const msg =
        action === "accept"
          ? data.emailed
            ? "Accepted — contractor notified"
            : "Bid accepted"
          : action === "reject"
            ? "Bid declined"
            : action === "complete"
              ? data.emailed
                ? "Completed — invoice requested"
                : "Marked completed"
              : "Voided";
      toast.success(msg);
      router.refresh();
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500">
            Send a scope to multiple contractors; each submits a bid you can accept or decline.
          </p>
        </div>
        {!showForm && (
      ...
      {showForm && (
        <form onSubmit={sendBlast} className="space-y-3 rounded-xl border border-gray-200 p-4 bg-gray-50">
          ...
          <div>
            ...
            >
              {showTerms ? "Hide" : "Edit"} bid terms
            </button>
            {showTerms && (
              <Textarea
                label="Terms the contractor agrees to when submitting"
                value={terms}
      ...
      {bidRequests.length === 0 && !showForm ? (
        ...
        <div className="space-y-3">
          {bidRequests.map((bid) => {
            const pending = bid.status === "submitted";
            return (
              <div
                key={bid.id}
                className={`rounded-xl border p-4 ${
                  pending ? "border-amber-300 bg-amber-50/40" : "border-gray-200"
                }`}
              >
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {bid.bid_amount != null && (
                        <span className="text-sm font-semibold text-gray-900">
                          {usd.format(bid.bid_amount)}
                        </span>
                      )}
                    </div>
                    <p className="text-sm text-gray-500 mt-0.5">
                      {bid.title}
                      {bid.status === "passed" && bid.decline_reason && ` · "${bid.decline_reason}"`}
                      {bid.status === "completed" && bid.invoice_requested_at && " · invoice requested"}
                    </p>
                    {bid.bid_note && pending && (
                      <p className="text-sm text-gray-600 mt-1 italic">“{bid.bid_note}”</p>
                    )}
                  </div>
                </div>
                ...
                <div className="flex flex-wrap gap-2 mt-3">
                  ...
                  {pending && (
                    <>
                      <button
                        onClick={() => act(bid, "accept")}
                        disabled={busy}
                        className="inline-flex items-center gap-1 text-xs px-3 py-1.5 rounded-lg border border-green-300 bg-green-600 text-white hover:bg-green-700 disabled:opacity-50"
                      >
                        <Check className="w-3.5 h-3.5" /> Accept
                      </button>
                      <button
                        onClick={() => act(bid, "reject")}
                        disabled={busy}
                        className="inline-flex items-center gap-1 text-xs px-3 py-1.5 rounded-lg border border-gray-300 text-gray-700 hover:bg-gray-50 disabled:opacity-50"
                      >
                  ...
                  {bid.status === "completed" && (
                    <span className="inline-flex items-center gap-1 text-xs px-3 py-1.5 rounded-lg border border-emerald-200 text-emerald-700">
                      <Receipt className="w-3.5 h-3.5" /> Invoice requested
                    </span>
                  )}
                  ...
                  {bid.document?.file_url && (
                    ...
                    >
                      <FileCheck2 className="w-3.5 h-3.5" /> Bid PDF
                    </a>
                  )}
